...
const UIComps = ({ bg, addNode, removeNode }: UICompProps) => {
  ...
  const [showComponents, setShowComponents] = useState<component[]>([
    ...components,
  ]);
  ...
  useEffect((): void => {
    if (search) {
      return setShowComponents(
        showComponents.filter((component) =>
  ...
  return (
    <div className="saved-comp-page">
      <div id="flow-info">
        <input
          id="flow-background-input"
          onChange={(e) => setBgColorIn(e.target.value)}
          value={bgColorIn}
        />
        <button
          id="flow-background-submit"
          onClick={() => setBgColor(bgColorIn)}
        >
          Change BG Color
        </button>
      </div>
